...
const Navbar = ({ onMenuClick }) => {
  ...
  return (
    <>
        ...
        <div className="w-full h-16 px-2 sm:px-3 lg:px-4 flex items-center justify-between">
          ...
          {user ? (
            <div className="hidden sm:flex items-center gap-3">
              <Link
                to="/chat"
                className="rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 px-5 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-900/30 transition-all hover:scale-[1.02]"
              >
                Open Workspace
              </Link>
            ...
            <div className="hidden sm:flex items-center gap-3">
              <Link
                to="/login"
                className="rounded-xl border border-[#1a2d4a] bg-[#0a1428] hover:bg-[#0f1e38] px-5 py-2.5 text-sm text-slate-300 hover:text-white transition"
              >
                Sign In
              </Link>
              ...
              <Link
                to="/register"
                className="rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-blue-900/30 transition-all hover:scale-[1.02]"
              >
                Get Started
              </Link>
            </div>
          ...
          <div className="sm:hidden flex items-center gap-2">
            {user ? (
              ...
              <div className="flex items-center gap-2">
                <Link
                  to="/login"
                  className="rounded-xl border border-[#1a2d4a] bg-[#0a1428] px-3 py-2 text-xs text-slate-300"
                >
                  Sign In
                </Link>
      ...
      {mobileMenu && user && (
        <div className="sm:hidden fixed top-16 left-0 right-0 z-50 bg-[#060d1a] border-b border-blue-500/20 shadow-2xl shadow-black/80">

          <div className="flex items-center gap-3 px-5 py-4 border-b border-[#1a2d4a] bg-[#080f1f]">
            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-700 flex items-center justify-center font-black text-white">
              {user?.name?.charAt(0)?.toUpperCase()}
            </div>
